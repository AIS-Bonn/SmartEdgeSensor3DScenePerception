...
const string PERSON_3D_TOPIC_PRED = "human_pose_estimation/persons_3d_pred";
...
const double g_max_sync_diff = 0.120; // seconds

static std::vector<Person2DOcclusionList::ConstPtr> g_skel_data;
...
static PersonCovList g_prevEstimates;

struct EdgeTPU_BodyParts_Openpose{
    ...
    static constexpr int kpParent[18]      = {-1, 0,    1,    2,    3,    1,    5,    6,    1,    8,    9,     1,    11,   12,    0,    0,    14,   15};
    static constexpr double limbLength[18] = {-1, 0.20, 0.15, 0.28, 0.25, 0.15, 0.28, 0.25, 0.51, 0.45, 0.445, 0.51, 0.45, 0.445, 0.05, 0.05, 0.10, 0.10}; // From H36M - Statistics (except 14 - 17) (https://github.com/microsoft/multiview-human-pose-estimation-pytorch)
    static constexpr double limbLSigma[18] = {-1, 0.07, 0.07, 0.10, 0.10, 0.07, 0.10, 0.10, 0.15, 0.10, 0.10,  0.15, 0.10, 0.10,  0.05, 0.05, 0.05, 0.05};
};

constexpr int EdgeTPU_BodyParts_Openpose::kpParent[18];
constexpr double EdgeTPU_BodyParts_Openpose::limbLength[18];
constexpr double EdgeTPU_BodyParts_Openpose::limbLSigma[18];

/*Limb Lengths (campus / shelf)
LLR  (0, 1)  : 0.4329043627272892,
ULR  (1, 2)  : 0.42869183903891644,
ULL  (3, 4)  : 0.4263273099293717,
LLL  (4, 5)  : 0.44251702349280214,
LAR  (6, 7)  : 0.27798603178892156,
UAR  (7, 8)  : 0.32440406997933496,
UAL  (9, 10) : 0.3146652849871362,
LAL  (10, 11): 0.2753950357339214,
Head (12, 13): 0.2649257335273264,
Hips (2, 3)  : 0.20922146798918162,
Shld (8, 9)  : 0.322364091322673,
'height'     : 0.6229562400815453,
'neck'       : 0.11247479785280212
*/
...
//static double g_predNoiseSigma = 0.10; // standard deviation for prediction noise covariance
...
struct PersonHypothesis{
  ...
  std::vector<std::vector<float> > keypoints_depth, keypoints_depth_sigma;
...
void getIntrinsics(vector<sensor_msgs::CameraInfo>& intrinsics, ros::NodeHandle& nh){
  ...
  for (const auto& intr : intrinsics) {
    cout << "fx: " << intr.K[0] << ", fy: " << intr.K[4] << ", cx: " << intr.K[2] << ", cy: " << intr.K[5] << ", res: " << intr.width << "x" << intr.height << endl;
  }
}
...
int normalize_keypoints(vector<Eigen::Vector3f>& keypoints_normalized, vector<Eigen::Matrix2f>& covs_normalized, const image_geometry::PinholeCameraModel& intr, const person_msgs::Person2DOcclusion& human){
  int num_valid_kps = 0;
  ...
  for (int kp_idx = 0; kp_idx < NUM_KEYPOINTS; ++ kp_idx) {
    ...
    if(kp.score >= g_triangulation_threshold){ // occluded keypoints have score 0 and will be excluded here.
...
double calcCost(const PersonHypothesis& hyp, const vector<Eigen::Vector3f>& det_kps, int det_ID, const vector<Eigen::Matrix3f>& fundamental_matrices,
                const vector<Eigen::Matrix<float, 3, 4> >& projection_matrices, const vector<float>& dets_kps_depth, const vector<float>& dets_kps_depth_sigma, bool& veto){
//  static constexpr float sqrt2pi = 2.506628274631f; //std::sqrt(2.f*M_PIf32);
  double total_cost = 0.;
  int n_obs_used = 0;
  ...
  for (int obs_idx = 0; obs_idx < n_obs_in_hyp; ++obs_idx) {
    ...
    const Eigen::Matrix<float, 3, 4>& P12 = projection_matrices[hyp.cameraIDs[obs_idx] * NUM_CAMERAS + det_ID];
    const Eigen::Matrix<float, 3, 4>& P21 = projection_matrices[det_ID * NUM_CAMERAS + hyp.cameraIDs[obs_idx]];
    const bool depth_p1 = !hyp.keypoints_depth[obs_idx].empty();
    const bool depth_p2 = !dets_kps_depth.empty();
    ...
    for (int kp_idx = 0; kp_idx < NUM_KEYPOINTS; ++ kp_idx) {
      if(hyp_kps[kp_idx].z() > g_triangulation_threshold && det_kps[kp_idx].z() > g_triangulation_threshold){ // z is confidence value! both keypoints are valid
        const Eigen::Vector3f p1(hyp_kps[kp_idx].x(), hyp_kps[kp_idx].y(), 1.0f); // hyp
        const Eigen::Vector3f p2(det_kps[kp_idx].x(), det_kps[kp_idx].y(), 1.0f); // det
        const Eigen::Vector3f epipolar_line_1 = F * p1;
        const Eigen::Vector3f epipolar_line_2 = FT * p2;
        //Point-to-line distance: abs(p2.dot(l)) / sqrt(l0²+l1²)

        const float epipolar_line_1_norm_sqr = epipolar_line_1(0) * epipolar_line_1(0) + epipolar_line_1(1) * epipolar_line_1(1);
        const float d1 = std::abs(p2.dot(epipolar_line_1)) / std::sqrt(epipolar_line_1_norm_sqr);
        float depth_factor1 = 1.f;
        if(depth_p1 && hyp.keypoints_depth[obs_idx][kp_idx] > 0){
          const float& z = hyp.keypoints_depth[obs_idx][kp_idx];
          const float z_min = std::max(0.f, hyp.keypoints_depth[obs_idx][kp_idx] - std::max(0.5f, hyp.keypoints_depth_sigma[obs_idx][kp_idx]));
          const float z_max = hyp.keypoints_depth[obs_idx][kp_idx] + std::max(0.5f, hyp.keypoints_depth_sigma[obs_idx][kp_idx]);
          Eigen::Matrix3f points3D; // project p1 with (min,mean,max) depth to cam2.
          points3D << p1.x() * z_min, p1.x() * z, p1.x() * z_max,
                      p1.y() * z_min, p1.y() * z, p1.y() * z_max,
                      z_min         , z         , z_max;
          const Eigen::Matrix3f pts_transformed_homog =  P21 * points3D.colwise().homogeneous();
          const Eigen::Matrix<float, 2, 3> pts_transformed = pts_transformed_homog.colwise().hnormalized();
          const Eigen::Vector2f& pt_depth_min = pts_transformed.col(0);
          const Eigen::Vector2f& pt_depth = pts_transformed.col(1);
          const Eigen::Vector2f& pt_depth_max = pts_transformed.col(2);

          // point closest to p2 on epipolar line.
          const float x_intersect = (epipolar_line_1(1) * (epipolar_line_1(1) * p2.x() - epipolar_line_1(0) * p2.y()) - epipolar_line_1(0) * epipolar_line_1(2)) / epipolar_line_1_norm_sqr; //b*(b x0 - a y0) - ac / (a^2 + b^2)
          const float y_intersect = (epipolar_line_1(0) * (-epipolar_line_1(1) * p2.x() + epipolar_line_1(0) * p2.y()) - epipolar_line_1(1) * epipolar_line_1(2)) / epipolar_line_1_norm_sqr; //a*(-b x0 + a y0) - bc / (a^2 + b^2)
          
//           // check that projected points actually are on epipolar line:
//           const float d1_depth_min = std::abs(Eigen::Vector3f(pt_depth_min.x(), pt_depth_min.y(), 1.f).dot(epipolar_line_1)) / std::sqrt(epipolar_line_1_norm_sqr);
//           const float d1_depth = std::abs(Eigen::Vector3f(pt_depth.x(), pt_depth.y(), 1.f).dot(epipolar_line_1)) / std::sqrt(epipolar_line_1_norm_sqr);
//           const float d1_depth_max = std::abs(Eigen::Vector3f(pt_depth_max.x(), pt_depth_max.y(), 1.f).dot(epipolar_line_1)) / std::sqrt(epipolar_line_1_norm_sqr);
//           const float d1_intersect = std::abs(Eigen::Vector3f(x_intersect, y_intersect, 1.f).dot(epipolar_line_1)) / std::sqrt(epipolar_line_1_norm_sqr);
//           cout << "\t obs_idx " << obs_idx << ", joint " << kp_idx << ": camera hyp: " << hyp.cameraIDs[obs_idx] << ", camera det: " << det_ID << ": epipolar dist 21 of proj. pts: min: " << d1_depth_min << ", mean: " << d1_depth << ", max: " << d1_depth_max << ", intersect: " << d1_intersect << endl;

          // distance along line to point projected via depth estimate
          const float line_dist_sqr = (pt_depth.x() - x_intersect) * (pt_depth.x() - x_intersect) + (pt_depth.y() - y_intersect) * (pt_depth.y() - y_intersect);
          const float sigma_depth_sqr = 2 * ((pt_depth_max.x() - pt_depth.x()) * (pt_depth_max.x() - pt_depth.x()) + (pt_depth_max.y() - pt_depth.y()) * (pt_depth_max.y() - pt_depth.y())
              + (pt_depth_min.x() - pt_depth.x()) * (pt_depth_min.x() - pt_depth.x()) + (pt_depth_min.y() - pt_depth.y()) * (pt_depth_min.y() - pt_depth.y()));

          //const float sigma_depth = sqrt(sigma_depth_sqr);
          depth_factor1 = std::min(1e3f, 0.9f * std::exp(line_dist_sqr / (2*sigma_depth_sqr))); // weight by inverse Gaussian // sqrt2pi * sigma_depth
          //cout << "\t obs_idx " << obs_idx << ", joint " << kp_idx << ": camera hyp: " << hyp.cameraIDs[obs_idx] << ", camera det: " << det_ID << ", epipolar dist 21: " << d1 << ", dist on line: " << std::sqrt(line_dist_sqr) << ", sigma depth: " << hyp.keypoints_depth_sigma[obs_idx][kp_idx] << ", depth factor: " << depth_factor1 << endl;
        }

        const float epipolar_line_2_norm_sqr = epipolar_line_2(0) * epipolar_line_2(0) + epipolar_line_2(1) * epipolar_line_2(1);
        const float d2 = std::abs(p1.dot(epipolar_line_2)) / std::sqrt(epipolar_line_2_norm_sqr);
        float depth_factor2 = 1.f;
        if(depth_p2 && dets_kps_depth[kp_idx] > 0){
          const float& z = dets_kps_depth[kp_idx];
          const float z_min = std::max(0.f, dets_kps_depth[kp_idx] - std::max(0.5f, dets_kps_depth_sigma[kp_idx]));
          const float z_max = dets_kps_depth[kp_idx] + std::max(0.5f, dets_kps_depth_sigma[kp_idx]);
          Eigen::Matrix3f points3D; // project p2 with (min,mean,max) depth to cam1.
          points3D << p2.x() * z_min, p2.x() * z, p2.x() * z_max,
                      p2.y() * z_min, p2.y() * z, p2.y() * z_max,
                      z_min         , z         , z_max;
          const Eigen::Matrix3f pts_transformed_homog =  P12 * points3D.colwise().homogeneous();
          const Eigen::Matrix<float, 2, 3> pts_transformed = pts_transformed_homog.colwise().hnormalized();
          const Eigen::Vector2f& pt_depth_min = pts_transformed.col(0);
          const Eigen::Vector2f& pt_depth = pts_transformed.col(1);
          const Eigen::Vector2f& pt_depth_max = pts_transformed.col(2);
          ...
          // point closest to p2 on epipolar line.
          const float x_intersect = (epipolar_line_2(1) * (epipolar_line_2(1) * p1.x() - epipolar_line_2(0) * p1.y()) - epipolar_line_2(0) * epipolar_line_2(2)) / epipolar_line_2_norm_sqr; //b*(b x0 - a y0) - ac / (a^2 + b^2)
          const float y_intersect = (epipolar_line_2(0) * (-epipolar_line_2(1) * p1.x() + epipolar_line_2(0) * p1.y()) - epipolar_line_2(1) * epipolar_line_2(2)) / epipolar_line_2_norm_sqr; //a*(-b x0 + a y0) - bc / (a^2 + b^2)
          
//           // check that projected points actually are on epipolar line:
//           const float d2_depth_min = std::abs(Eigen::Vector3f(pt_depth_min.x(), pt_depth_min.y(), 1.f).dot(epipolar_line_2)) / std::sqrt(epipolar_line_2_norm_sqr);
//           const float d2_depth = std::abs(Eigen::Vector3f(pt_depth.x(), pt_depth.y(), 1.f).dot(epipolar_line_2)) / std::sqrt(epipolar_line_2_norm_sqr);
//           const float d2_depth_max = std::abs(Eigen::Vector3f(pt_depth_max.x(), pt_depth_max.y(), 1.f).dot(epipolar_line_2)) / std::sqrt(epipolar_line_2_norm_sqr);
//           const float d2_intersect = std::abs(Eigen::Vector3f(x_intersect, y_intersect, 1.f).dot(epipolar_line_2)) / std::sqrt(epipolar_line_2_norm_sqr);
//           cout << "\t obs_idx " << obs_idx << ", joint " << kp_idx << ": camera hyp: " << hyp.cameraIDs[obs_idx] << ", camera det: " << det_ID << ": epipolar dist 12 of proj. pts: min: " << d2_depth_min << ", mean: " << d2_depth << ", max: " << d2_depth_max << ", intersect: " << d2_intersect << endl;
          ...
          //const float sigma_depth = sqrt(sigma_depth_sqr);
          depth_factor2 = std::min(1e3f, 0.9f * std::exp(line_dist_sqr / (2*sigma_depth_sqr))); // weight by inverse Gaussian // sqrt2pi * sigma_depth
          //cout << "\t obs_idx " << obs_idx << ", joint " << kp_idx << ": camera hyp: " << hyp.cameraIDs[obs_idx] << ", camera det: " << det_ID << ", epipolar dist 12: " << d2 << ", dist on line: " << std::sqrt(line_dist_sqr) << ", sigma depth: " << dets_kps_depth_sigma[kp_idx] << ", depth factor: " << depth_factor2 << endl;
        }

        cost += static_cast<double>(depth_factor1 * d1 + depth_factor2 * d2);
        ++n_joints_used;
      }
    }
    ...
    if(n_joints_used > 0){
      ...
      //cout << "\t\t obs " << obs_idx << "<--> det: cost: " << cost << endl;
    }
    else {
      //cout << "\t\t obs " << obs_idx << "<--> det: no mutually observed joints." << endl;
    }
  }
...
Eigen::Vector3f triangulate(const std::vector<Matrix34f>& cameraMatrices, const std::vector<Eigen::Vector3f>& jointOnEachCamera, bool weight_by_conf = false, double* reproj_error = nullptr){ // jointOnEachCamera are [x, y, conf]
  ...
  //const Eigen::Vector4f pt_homog = Eigen::SelfAdjointEigenSolver<Eigen::Matrix4f>(ATA).eigenvectors().col(0);
  ...
  //cout << "homog. res. eigen: " << pt_homog.transpose() << endl << endl;
...
Eigen::RowVectorXf draw_sigma_points(vector<vector<Eigen::Vector3f> >& sigma_points, const vector<Eigen::Vector3f>& jointOnEachCamera, const vector<Eigen::Matrix2f>& cov_mats){
  ...
  constexpr float kappa = 0.5; // (scaling) equal weight on all samples // TODO: find best parameter (or use scaled transform, kappa=0, alpha = 1e-3, beta = 2)
...
//  const int num_cameras = cameraMatrices.size();
//  if(num_cameras < 4)
//    cout << "; triangulated using: " << num_cameras << " cameras. transformed samples:" << endl;
...
//  transformedSamples.col(0) = mean_sample;
...
void triangulate_persons(const vector<Person2DOcclusionList::ConstPtr>& people, PersonCovList& persons3d_msg, PersonCovList& persons3d_pred_msg, visualization_msgs::MarkerArray& skel3d_msg,
                        const map<string, Matrix34f>& transforms_cam, const vector<Eigen::Matrix3f>& fundamental_matrices, const vector<Eigen::Matrix<float, 3, 4> >& projection_matrices, const vector<Eigen::Vector3f>& ground_normals_sensor, const vector<sensor_msgs::CameraInfo>& intrinsics){ //, double delta_t
  ...
  vector<vector<person_msgs::Person2DOcclusion>> humans;
  ...
  for (int i = 0; i < NUM_CAMERAS; ++i) { // This loop basically serves to sort out camera views with no detection at all.
    if(!people[i]->persons.empty()){ // at least one human detected in camera view i.

      // Remove totally / too much occluded persons
      vector<person_msgs::Person2DOcclusion> persons_cam_i = people[i]->persons;
      //TODO: eventually replace occluded keypoints with their original detections, to simulate no occlusion information available..
      persons_cam_i.erase(std::remove_if(persons_cam_i.begin(), persons_cam_i.end(), [](const person_msgs::Person2DOcclusion& pers){return pers.score <= 0.f || pers.n_valid - pers.n_occluded <= NUM_KEYPOINTS / 2;}), persons_cam_i.end());

      if(!persons_cam_i.empty()){
        ...
        const float fy = static_cast<float>(cam_intrinsics.fy());

        cameraIDs.push_back(i);

        for(auto & person : persons_cam_i){
          if(person.depth_est.empty() && // No local RGB-D depth available
             (person.keypoints[EdgeTPU_BodyParts_Simple::LShoulder].score > 0 || (person.n_occluded > 0 && person.occluded[EdgeTPU_BodyParts_Simple::LShoulder])) &&
             (person.keypoints[EdgeTPU_BodyParts_Simple::RShoulder].score > 0 || (person.n_occluded > 0 && person.occluded[EdgeTPU_BodyParts_Simple::RShoulder])) &&
             (person.keypoints[EdgeTPU_BodyParts_Simple::LHip].score > 0 || (person.n_occluded > 0 && person.occluded[EdgeTPU_BodyParts_Simple::LHip])) &&
             (person.keypoints[EdgeTPU_BodyParts_Simple::RHip].score > 0 || (person.n_occluded > 0 && person.occluded[EdgeTPU_BodyParts_Simple::RHip]))) // both shoulders and hips are detected, or replaced from occlusion feedback
          {
            const int num_kps = person.keypoints.size();
            person.depth_est.resize(num_kps, 0);
            person.depth_sigma.resize(num_kps, 0);

            const float bbox_cx = (person.bbox[2] + person.bbox[0]) / 2.f;
            const float bbox_cy = (person.bbox[3] + person.bbox[1]) / 2.f;
            const auto ray_bbox = cam_intrinsics.projectPixelTo3dRay(cv::Point2d(bbox_cx, bbox_cy));
            const Eigen::Vector3f ray_bbox_normalized = Eigen::Vector3f((float)ray_bbox.x, (float)ray_bbox.y, (float)ray_bbox.z).normalized();
            const float sin_theta_bbox = ray_bbox_normalized.cross(ground_normals_sensor[i]).norm();

            const float neck_y = (person.keypoints[EdgeTPU_BodyParts_Simple::LShoulder].y + person.keypoints[EdgeTPU_BodyParts_Simple::RShoulder].y) / 2.f;
            const float pelvis_y = (person.keypoints[EdgeTPU_BodyParts_Simple::LHip].y + person.keypoints[EdgeTPU_BodyParts_Simple::RHip].y) / 2.f;
            const float torso_height = pelvis_y - neck_y;
            const float bbox_height = person.bbox[3] - person.bbox[1];

            float z = 0, z_sigma = 0;
            if (torso_height > 0 && std::abs(torso_height / bbox_height - 0.3f) < 0.15f){
              z = sin_theta_bbox * 0.55f / torso_height * fy;
              z_sigma = 0.15f / torso_height * fy;
            }

            for (int kp_idx = 0; kp_idx < num_kps; ++kp_idx) {
              const auto& kp = person.keypoints[kp_idx];
              if ((kp.score <= 0. && !(person.n_occluded > 0 && person.occluded[kp_idx])) || z <= 0.)
                continue;

              const auto ray = cam_intrinsics.projectPixelTo3dRay(cv::Point2d(kp.x, kp.y));
              const Eigen::Vector3f ray_normalized = Eigen::Vector3f((float)ray.x, (float)ray.y, (float)ray.z).normalized();
              const float sin_theta = ray_normalized.cross(ground_normals_sensor[i]).norm();
              const float cos_theta = ray_normalized.dot(ground_normals_sensor[i]);
              const float z_kp_factor = 1.f - (kp.y - bbox_cy) / fy * cos_theta / sin_theta;

              person.depth_est[kp_idx] = z * z_kp_factor;
              person.depth_sigma[kp_idx] = z_sigma; // * z_kp_factor;
            }
          }
        }

        humans.push_back(persons_cam_i);
        num_humans+=persons_cam_i.size();
      }
    }
  }
  ...
  //cout << endl << "Received " << num_humans << " humans as input to data association" << endl;
  ...
  while(H.size() == 0 && cam_idx < num_cams_with_det){
    ...
    for (int human_idx = 0; human_idx < humans[cam_idx].size(); ++human_idx) {
      ...
      int num_valid_kps = normalize_keypoints(keypoints_normalized, covs_normalized, intr, humans[cam_idx][human_idx]); // This will exclude occluded keypoints, as their score is 0.0
      if(num_valid_kps > NUM_KEYPOINTS / 2){ // only process hypotheses with more than half of the keypoints detected. TODO: maybe this can be relaxed when using occlusion information ?
        PersonHypothesis hyp;
        hyp.cameraExtrinsics.push_back(cameraExtrinsics[cam_idx]);
        hyp.cameraIDs.push_back(cameraIDs[cam_idx]);
        hyp.score.push_back(humans[cam_idx][human_idx].score); //TODO: impose minimum score ?!
        ...
        hyp.keypoints_depth.push_back(humans[cam_idx][human_idx].depth_est);
        hyp.keypoints_depth_sigma.push_back(humans[cam_idx][human_idx].depth_sigma);
        H.push_back(hyp);
//        if (humans[cam_idx][human_idx].depth_est.size() > 0)
//          cout << "Camera " << cam_idx << " (ID: " << hyp.cameraIDs.back() << "): added initial hypothesis with " << num_valid_kps << " valid keypoints and score: " << hyp.score.back() << " and " << humans[cam_idx][human_idx].depth_est.size() << " depth estimates" << endl;
      }
    }
    ++cam_idx;
  }
  ...
  for (; cam_idx < num_cams_with_det; ++cam_idx) {
    ...
    vector<vector<float> > dets_keypoints_depth, dets_keypoints_depth_sigma;
    const auto& intr = cameraIntrinsics[cam_idx];
    ...
    for (int det_idx = 0; det_idx < humans[cam_idx].size(); ++det_idx) {
      ...
      if(num_valid_kps > NUM_KEYPOINTS / 2){ // only process hypotheses with at least half of the keypoints detected
        ...
        dets_keypoints_depth.push_back(humans[cam_idx][det_idx].depth_est);
        dets_keypoints_depth_sigma.push_back(humans[cam_idx][det_idx].depth_sigma);
//        if (humans[cam_idx][det_idx].depth_est.size() > 0)
//          cout << "Camera " << cam_idx << " (ID: " << cameraIDs[cam_idx] << "): detection with " << num_valid_kps << " valid keypoints and score: " << dets_score.back() << " and " << humans[cam_idx][det_idx].depth_est.size() << " depth estimates" << endl;
      }
    }
    ...
    //#pragma omp parallel for collapse(2) num_threads(n_det * n_hyp)
    for (int det_idx = 0; det_idx < n_det; ++det_idx) {
      for (int hyp_idx = 0; hyp_idx < n_hyp; ++ hyp_idx) {
        bool veto;
        C(hyp_idx, det_idx) = calcCost(H[hyp_idx], dets_keypoints_normalized[det_idx], cameraIDs[cam_idx], fundamental_matrices, projection_matrices, dets_keypoints_depth[det_idx], dets_keypoints_depth_sigma[det_idx], veto);
        if(!veto && C(hyp_idx, det_idx) < g_max_epipolar_error){
          mask(hyp_idx, det_idx) = true;
          assignment[hyp_idx] = det_idx;
        }

        //cout << "\t hyp " << hyp_idx << " <-> det " << det_idx << ": cost: " << C(hyp_idx, det_idx) << ", veto: " << (int) !mask(hyp_idx, det_idx) << " (" << veto << ")." << endl;
      }
    }
    ...
    //  cout << "Hypothesis " << i << " got assigned detection: " << assignment(i) << endl;
    //}
    ...
    for (int hyp_idx = 0; hyp_idx < n_hyp; ++hyp_idx) {
      ...
      if(det_idx >= 0){ // this hypothesis was assigned a detection
        ...
        if(!mask(hyp_idx, det_idx)){ // even the closest other person has too large epipolar distance (> threshold) -> new hypothesis
          ...
          hyp.keypoints_depth.push_back(dets_keypoints_depth[det_idx]);
          hyp.keypoints_depth_sigma.push_back(dets_keypoints_depth_sigma[det_idx]);
          H.push_back(hyp);
          ...
          H[hyp_idx].keypoints_depth.push_back(dets_keypoints_depth[det_idx]);
          H[hyp_idx].keypoints_depth_sigma.push_back(dets_keypoints_depth_sigma[det_idx]);
...
//    cout << "triangulation of hypothesis " << hyp_idx << " with " << H[hyp_idx].cameraIDs.size() << " observations." << endl;
//  }

  #pragma omp parallel num_threads(H.size())
  {
    vector<PersonCov> persons3d_private, persons3d_pred_private;
    ...
    for (int hyp_idx = 0; hyp_idx < H.size(); ++hyp_idx) {
      ...
      if(n_obs >= 2){ // Triangulate each joint
        ...
        skel3d_single.id = hyp_idx; // TODO: Track-ID Association over time
        ...
        skel3d_joints.id = hyp_idx; // TODO: Track-ID Association over time
        ...
        for (int kp_idx = 0; kp_idx < NUM_KEYPOINTS; ++ kp_idx) {
          ...
          if (reprojError > g_reproj_error_max_acceptable && numCameras == 3){
            //cout << "Joint " << kp_idx << ": num cameras: " << numCameras << ", avg reprojection error: " << reprojError << " (approx. " << reprojError * cameraIntrinsics[0].fx() << "px), score: " << avg_score << endl;
            int bestIndex = -1;
            ...
            //cout << "Joint " << kp_idx << ": avg reprojection error: " << reprojError << " (approx. " << reprojError * cameraIntrinsics[0].fx() << "px), score: " << avg_score << endl;
          ...
          if (reprojError > g_reproj_error_max_acceptable){ // if reprojection error is still large, down-weight the measurement
            avg_score *= (g_reproj_error_max_acceptable / reprojError); //TODO use exponential weighting or exclude joints in this case ?
...
//        else {
//          cout << "WARNING: Cannot check for outliers as root joint is undefined." << endl;
//        }
          ...
          for (int kp_idx = 0; kp_idx < FUSION_BODY_PARTS::NUM_KEYPOINTS; ++kp_idx) {
            ...
            if(kp.score > 0){
              ...
              if(joint_dist > g_max_joint_dist_to_root){
                //ROS_WARN("Resetting keypoint %d, as it has large distance to root: %fm", kp_idx, joint_dist);
        ...
        if(feet_height < -0.50 || feet_height > 0.95){ // Max tolerance of 50cm below or 95cm above ground
        ...
        if(num_valid_keypoints > g_min_num_valid_keypoints){
          ...
          persons3d_pred_private.push_back(person_3d); // TODO prediction (?)
    ...
    #pragma omp critical
    {
      ...
      persons3d_pred_msg.persons.insert(persons3d_pred_msg.persons.end(), persons3d_pred_private.begin(), persons3d_pred_private.end());
  ...
  for (int i = 0; i < persons3d_msg.persons.size(); ++i) {
    for (int j = i+1; j < persons3d_msg.persons.size();) {
      if(calc_3D_dist(persons3d_msg.persons[i], persons3d_msg.persons[j]) < g_merge_dist_thresh){ // merge person_3d with p
        ...
        merge_persons(persons3d_pred_msg.persons[i], persons3d_pred_msg.persons[j]);

        persons3d_msg.persons.erase(persons3d_msg.persons.begin() + j);
        persons3d_pred_msg.persons.erase(persons3d_pred_msg.persons.begin() + j);
  ...
  g_prevEstimates = persons3d_msg; // TODO prediction (?)
  g_initialized = true; // TODO prediction (?)
}

void skeletonCallback(const vector<Person2DOcclusionList::ConstPtr>& people){
  //ROS_INFO("skeltonCallback, people size: %zu", people.size());
  //static double last_stamp = 0;
  //cout << "Received message from synchronizer, delta t0: " << people[0]->header.stamp.toSec() - last_stamp << "s." << endl;
  //last_stamp = people[0]->header.stamp.toSec();
  {
...
void skeletonThreadCallback(const map<string, Matrix34f>& transforms_cam, const vector<Eigen::Matrix3f>& fundamental_matrices, const vector<Eigen::Matrix<float, 3, 4> >& projection_matrices, const vector<Eigen::Vector3f>& ground_normals_sensor,
                            const vector<sensor_msgs::CameraInfo>& intrinsics, const ros::Publisher& pers3d_pub, const ros::Publisher& pers3d_pub_pred, const ros::Publisher& skel3d_pub){
  double last_stamp = 0;
  vector<Person2DOcclusionList::ConstPtr> people;
  vector<Person2DOcclusionList::Ptr> dummy_msgs(NUM_CAMERAS);
  for (int i = 0; i < NUM_CAMERAS; ++i) {
    dummy_msgs[i] = boost::make_shared<Person2DOcclusionList>();
  }
  ...
  while(ros::ok()){
    ...
    //cout << "Processing message, delta t0: " << delta_t <<"s." << endl;
    if(delta_t > 0.17){
...
//    vector<double> deltas_print;
//    vector<string> strings_print;
//    bool warn_sync_diff = false;
//    for (int i = 0; i < NUM_CAMERAS; ++i) {
//      for(int j = i+1; j < NUM_CAMERAS; ++j){
//        double dt = (people[i]->header.stamp - people[j]->header.stamp).toSec();
//        if(std::abs(dt) > g_max_sync_diff){
//          strings_print.push_back("t_" + std::to_string(i) + "_" + std::to_string(j));
//          deltas_print.push_back(dt);
//          warn_sync_diff = true;
//        }
//      }
//    }
//    if(warn_sync_diff){
//      cout << "WARNING: sync time diff larger than " << g_max_sync_diff * 1000 << "ms: ";
//      for (int k = 0; k < deltas_print.size(); ++k) {
//        cout << strings_print[k] << ": " << deltas_print[k] * 1000 << "ms, ";
//      }
//      cout << endl;
//    }
    ...
    PersonCovList persons3d_pred_msg = persons3d_msg;

    visualization_msgs::MarkerArray skel3d_msg; // visualization message

    triangulate_persons(people, persons3d_msg, persons3d_pred_msg, skel3d_msg, transforms_cam, fundamental_matrices, projection_matrices, ground_normals_sensor, intrinsics); //, delta_t

    pers3d_pub.publish(persons3d_msg);
    pers3d_pub_pred.publish(persons3d_pred_msg);
...
int main (int argc, char** argv)
{
  ...
  g_triangulation_threshold = (g_param_pose_method == "openpose") ? 0.20f : 0.30f;
  ...
  if(g_param_pose_method == "h36m"){
    ...
    g_kpParent = EdgeTPU_BodyParts_Openpose::kpParent;
    g_limbLength = EdgeTPU_BodyParts_Openpose::limbLength;
    g_limbLSigma = EdgeTPU_BodyParts_Openpose::limbLSigma;
  ...
  ros::Publisher pub_pers3d_pred = nh.advertise<PersonCovList>(PERSON_3D_TOPIC_PRED, 1);

  ros::Publisher pub_skel3d = nh.advertise<visualization_msgs::MarkerArray>(SKELETON_3D_TOPIC, 1);

  std::vector<message_filters::Subscriber<Person2DOcclusionList>> skel_subs(NUM_CAMERAS);
  for (int i = 0; i < NUM_CAMERAS; ++i) {
  ...
  vector<Eigen::Vector3f> ground_normals_sensor;
  for (int i = 0; i < NUM_CAMERAS; ++i) {
    ...
    ground_normals_sensor.push_back((transforms_cam.at(CAM_FRAMES[i]).linear() * Eigen::Vector3d(0, 0, 1)).normalized().cast<float>());
  }
  ...
  vector<Eigen::Matrix<float, 3, 4> > projection_matrices(NUM_CAMERAS*NUM_CAMERAS, Eigen::Matrix<float, 3, 4>::Identity());
  for (int i = 0; i < NUM_CAMERAS; ++i) {
//    Eigen::Matrix3d K_i; // Not necessary, as projection will be computed in normalized camera coordinates.
//    K_i << intrinsics[i].K[0], 0.0, intrinsics[i].K[2], 0.0, intrinsics[i].K[4], intrinsics[i].K[5], 0.0, 0.0, 1.0;
    for(int j = 0; j < NUM_CAMERAS; ++j){
      projection_matrices[i * NUM_CAMERAS + j] = ((transforms_cam.at(CAM_FRAMES[i]) * transforms_cam.at(CAM_FRAMES[j]).inverse()).affine()).cast<float>(); // Pij
    }
  }
  //cout << projection_matrices[0] << endl << endl << projection_matrices[1] << endl << endl << projection_matrices[NUM_CAMERAS] << endl << endl;

  ROS_INFO("Synchronizing %d cameras, calculated %zu fundamental matrices and %zu projection matrices", NUM_CAMERAS, fundamental_matrices.size(), projection_matrices.size());
  ...
  std::thread skel_data_thread(std::bind(skeletonThreadCallback, std::cref(camera_matrices), std::cref(fundamental_matrices), std::cref(projection_matrices), std::cref(ground_normals_sensor), std::cref(intrinsics), std::cref(pub_pers3d), std::cref(pub_pers3d_pred), std::cref(pub_skel3d)));

  typedef message_filters::sync_policies::ApproximateTimeVec<Person2DOcclusionList> mySyncPolicy;
